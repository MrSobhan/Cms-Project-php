...
try {
    if (isset($_POST['sub'])) {
        ...
        $queryinsert = "UPDATE `users` SET`name`=?,`img`=?,`password`=?,`email`=?,`type`=? WHERE id=?";
        $statusment1 = $pdo->prepare($queryinsert);
        $statusment1->execute([$name_user, $img_user, $pass_user, $email_user, $type_user , $id]);
                    ...
                    <div class="row">
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item"><a href="./index.html">لیست کاربران</a></li>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form action="#" method="post" enctype="multipart/form-data">
                                    ...
                                    <center><img src="<?= '../ProfileUsers/' . $users[0]->img ?>" alt="" class="img-fluid w-25 rounded-pill img-js"></center>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="name_user" class="form-control" value="<?= $users[0]->name?>">
                                    </div>

                                    <div class="mb-5">
                                        <label for="exampleInputEmail2" class="form-label">گذرواژه</label>
                                        <input type="text" name="pass_user" class="form-control" value="<?= $users[0]->password?>">
                                    </div>

                                    <select name="type_user" class="form-select mb-3" aria-label="Default select example">
                                        <option <?= ($users[0]->type == 'admin') ? "selected": "" ?> value="admin">admin</option>
                                        <option <?= ($users[0]->type == 'user') ? "selected": "" ?> value="user">user</option>
                                    </select>

                                    <div class="mb-3">
                                        <label for="exampleInputEmail2" class="form-label">ایمیل</label>
                                        <input type="text" name="email_user" class="form-control" value="<?= $users[0]->email?>">
                                    </div>
